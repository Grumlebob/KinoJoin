...
public class JoinEventService(KinoContext context) : IJoinEventService
{
    public async Task<int> PutAsync(UpsertJoinEventDto joinEventDto)
    {
        ...
        var newRooms = joinEventWithNavProps.Showtimes.Select(st => st.Room).DistinctBy(r => r.Id).ToList();
        ...
        try
        {
            ...
            {
                showtime.PlaytimeId =
                    (await context.Playtimes.FirstAsync(p => p.StartTime == showtime.Playtime.StartTime)).Id;
                showtime.VersionTagId = (await context.Versions.FirstAsync(v => v.Type == showtime.VersionTag.Type)).Id;
            }
            ...
            else
            {
                ...
                joinEventWithNavProps.SelectOptions.RemoveAll(s => voteOptions.Contains(s.VoteOption) && colorOptions.Contains(s.Color));
