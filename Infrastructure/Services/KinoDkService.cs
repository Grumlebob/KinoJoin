...
public class KinoDkService : IKinoDkService
{
        ...
        ICollection<int>? genreIds = null, DateTime? fromDate = null, DateTime? toDate = null)
    {
        ...
        var cinemaIdsToNames =
            apiResultObject.Content.Facets.Cinemas.Options.ToDictionary(cinemaOption => cinemaOption.Key,
                cinemaOption => cinemaOption.Value);
        var movieIdsToNames =
            apiResultObject.Content.Facets.Movies.Options.ToDictionary(movieOption => movieOption.Key,
        ...
        foreach (var jsonCinema in apiResultObject.Content.Content.Content)
        {
                                 ...
                                 .Id))) //if not contains key it is not a movie (there are events with different ids)
            {
                ...
                {
                    if (jsonVersion.Label.Contains("lukket forestilling", StringComparison.CurrentCultureIgnoreCase))
                    {
                        continue;
                    }
        ...
        var index = 0;
        foreach (var movieId in movieIds.Where(movieId => showtimes.All(f => f.Movie.Id != movieId)))
        {
            filterString.Append($"&movies[{index++}]={movieId}");
        }

        if (filterString.Length == 0) return (showtimes, []); //all movies had showtimes
        ...
        if (root == null) return (showtimes, []);

        List<Movie> missingMovies = [];
        foreach (var movie in root.Content.Content.Content.SelectMany(cinema => cinema.Movies
                     .Where(movie => missingMovies.All(m => m.Id != movie.Id))))
        {
            if (!int.TryParse(movie.Content.FieldPlayingTime, out var duration)) duration = 0;
