...
public class MonkeyTests : IAsyncLifetime
{
    ...
    private readonly Faker<Domain.Entities.Monkey> _monkeyGenerator = new Faker<Domain.Entities.Monkey>()
    ...
    public async Task CreateGetDeleteMonkeyIntegrationFlow()
    {
        ...
        {
            var createResponse = await _client.PostAsJsonAsync("/monkeys", new CreateMonkeyDto(monkey.Name, monkey.Age));
        ...
        var resultAfterDelete = await GetMonkeysResponseAfterDelete.Content.ReadFromJsonAsync<IEnumerable<MonkeyDto>>();
